...
namespace Server.MirDatabase
{
    public class CharacterInfo
    {
        ...
        public DateTime VIP = DateTime.MinValue;

        public bool HumUp = false;

        public bool Thrusting, HalfMoon, CrossHalfMoon;
        public bool DoubleSlash;
        public bool CrescentSlash;
        public byte MentalState;
        public byte MentalStateLvl;

        public int HeroLoadVersion = 112; //JP bypass to fix PETE's shit
        ...
        public List<PlayerRecipes> MyRecipes = new List<PlayerRecipes>();
        public bool HasHero = false;
        public sbyte CurrentHeroCount = 1;
        public sbyte HeroCapCount = 9;
        ...
        public void Save(BinaryWriter writer)
        {
            ...
            writer.Write(CurrentQuests.Count(x => !x.Info.HeroQuest));
            for (int i = 0; i < CurrentQuests.Count; i++)
                if(!CurrentQuests[i].Info.HeroQuest)
            ...
            writer.Write(HumUp);
            writer.Write(MyRecipes.Count);
            for (int i = 0; i < MyRecipes.Count; i++)
            {                        
                MyRecipes[i].Save(writer);
            }
            writer.Write(HasHero);

            writer.Write(HeroLoadVersion);
            writer.Write(ClientCompletedQuests.Count);
            for (int i = 0; i < ClientCompletedQuests.Count; i++)
                ClientCompletedQuests[i].Save(writer);
            writer.Write(CurrentHeroCount);
            writer.Write(HeroCapCount);
