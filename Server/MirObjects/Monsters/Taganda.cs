        ...
        public void DoSwingAttack()
        {
            ...
            for (int i = 0; i < locs.Length; i++)
            {
                if (!CurrentMap.ValidPoint(locs[i]))
                    continue;
                Cell cell = CurrentMap.GetCell(locs[i]);
                if (cell != null &&
                    cell.Objects != null &&
                    cell.Objects.Count > 0)
                {
                    ...
                    {
                        switch(cell.Objects[x].Race)
                        {
                            case ObjectType.Player:
                            case ObjectType.Monster:
                                if (!cell.Objects[x].IsAttackTarget(this))
                                    continue;
                                int delay = Functions.MaxDistance(CurrentLocation, cell.Objects[x].CurrentLocation) * 50 + 500; //50 MS per Step
                                DelayedAction action = new DelayedAction(DelayedType.Damage, Envir.Time + delay, cell.Objects[x], GetAttackPower(MinDC, MaxDC), DefenceType.Agility);
