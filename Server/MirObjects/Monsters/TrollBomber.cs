...
{
    public class TrollBomber : MonsterObject
    {
        ...
        protected override void Attack()
        {
            ...
            else
            {
                ...
                if (Target is PlayerObject tmpOb) // this function is single target ye
                {
                    ...
                    {
                        if (Envir.Random.Next(24) == 0 && tmpOb.Class == MirClass.Warrior && tmpOb.Class == MirClass.Assassin)
                        {
                            tmpOb.ApplyPoison(new Poison { Owner = this, PType = PoisonType.Green, Duration = GetAttackPower(MinDC, MaxDC), TickSpeed = 1000 }, this);
                        }
                        ...
                        if (Envir.Random.Next(30) == 0 && tmpOb.Class == MirClass.Taoist)
                        {
                            tmpOb.ApplyPoison(new Poison { Owner = this, PType = PoisonType.Burning, Duration = GetAttackPower(MinSC, MaxSC), TickSpeed = 1000 }, this);
                        }
                    }
                }
            }
