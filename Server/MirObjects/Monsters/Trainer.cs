...
namespace Server.MirObjects.Monsters
{
    public class Trainer : MonsterObject
    {
        ...
        public override void PoisonDamage(int damage, MapObject attacker)
        {
            ...
            if (attacker.Race != ObjectType.Player) return;
            ...
            if (_currentAttacker.Race == ObjectType.Player)
                _currentAttacker.ReceiveChat(string.Format("{1} inflicted {0} Damage, Dps: {2:#.00}.", damage, "Your poison", Dps), ChatType.Trainer);
            else if (_currentAttacker.Master != null &&
                _currentAttacker.Master.Race == ObjectType.Player)
                _currentAttacker.Master.ReceiveChat(string.Format("{1} inflicted {0} Damage, Dps: {2:#.00}.", damage, "Your poison", Dps), ChatType.Trainer);
            Poisoned = true;
        }
