...
namespace Server.MirObjects
{
    public sealed class NPCObject : MapObject
    {
        ...
        public const string
            ...
            BuyUsedKey = "[@BUYUSED]",
            CreateHeroKey = "[@CREATEHERO]",
            SocketKey = "[@SOCKETING]",
            PlayerRankKey = "[@PLAYERRANKLIST]",
            HeroRankKey = "[@HERORANKLIST]",
            LMSBRKey = "[@LMSBR]";
        ...
        public void ClearInfo()
        {
            ...
            if (Info.IsDefault)            
                SMain.Envir.CustomCommands = new List<string>();            
        }
        ...
        private NPCSegment ParseSegment(NPCPage page, IEnumerable<string> scriptLines)
        {
            ...
            int SayType = -1;
            int HelpX = -1 , HelpY = -1 , ExitX = -1, ExitY = -1 , QuestX = -1, QuestY = -1;
            ...
            for (int i = 0; i < lines.Count; i++)
            {
                ...
                if (lines[i].StartsWith("#"))
                {
                    ...
                    //extract page image
                    if (action[0].Contains("ELSESAY"))
                    {
                        string[] output = action[0].Split('(',',', ')');
                        if (output.Length > 1)
                        {
                            int.TryParse(output[1], out SayType);
                            action[0] = action[0].Split('(')[0];
                        }
                        if (output.Length > 3)
                        {
                            int.TryParse(output[2], out HelpX);
                            int.TryParse(output[3], out HelpY);

                        }
                        if (output.Length > 5)
                        {
                            int.TryParse(output[4], out ExitX);
                            int.TryParse(output[5], out ExitY);

                        }
                        if (output.Length > 7)
                        {
                            int.TryParse(output[6], out QuestX);
                            int.TryParse(output[7], out QuestY);

                        }


                    }
                    else if (action[0].Contains("SAY"))
                    {
                        string[] output = action[0].Split('(', ',', ')');
                        if (output.Length > 1)
                        {
                        ...
                        if (output.Length > 3)
                        {
                            int.TryParse(output[2], out QuestX);
                            int.TryParse(output[3], out QuestY);
                        ...
                        if (output.Length > 7)
                        {
                            int.TryParse(output[6], out HelpX);
                            int.TryParse(output[7], out HelpY);
            ...
            NPCSegment segment = new NPCSegment(page, say, buttons, elseSay, elseButtons, gotoButtons, SayType)
            {
                questX = QuestX,
                questY = QuestY,
                exitX = ExitX,
        ...
        public void Call(PlayerObject player, string key)
        {
            ...
            if (!player.NPCDelayed)
            {
                if (key != MainKey) // && ObjectID != player.DefaultNPC.ObjectID
                {
                    ...
                    {
                        if (!player.NPCSuccess.TryGetValue(segment, out bool result)) break; //no result for segement ?
            ...
            for (int i = 0; i < NPCPages.Count; i++)
            {
                ...
                int tempWindow = -1,questX = -1,questY = -1 , exitY = -1, exitX = -1;
                foreach (NPCSegment segment in page.SegmentList)
                {
                    if (page.BreakFromSegments)
                    {
                    ...
                    if (segment.WindowImage != -1)
                    {
                        tempWindow = segment.WindowImage;
                        questX = segment.questX;
                        questY = segment.questY;
                        exitX = segment.exitX;
                        exitY = segment.exitY;

                    }
                }
                Response(player, page, tempWindow,questX,questY,exitX,exitY);
            }
        ...
        private void Response(PlayerObject player, NPCPage page , int window , int questX,int questY, int exitX, int exitY)
        {
            player.Enqueue(new S.NPCResponse { Page = player.NPCSpeech, WindowImage = window, questX = questX, questY = questY , exitX = exitX, exitY = exitY });
        ...
        private void ProcessSpecial(PlayerObject player, NPCPage page)
        {
            ...
            switch (page.Key.ToUpper())
            {
                ...
                case MarketKey:
                    ...
                    player.GetMarket(string.Empty, ItemType.Nothing, 0, int.MaxValue);
                    break;
                ...
                case ResetKey:
                    ...
                    break;
                case CreateHeroKey:
                    player.Enqueue(new S.HeroCreation() {allowSin = Envir.heroConfig.allowSin , allowArc = Envir.heroConfig.allowArc });
                    break;
                case PearlBuyKey:
                    /*
                    for (int i = 0; i < Goods.Count; i++)
                    ...
                    break;
                case SocketKey:
                    player.Enqueue(new S.OpenSocketDialog());
                    break;
                case GtBoardKey:
                    player.Enqueue(new S.OpenGuildBoardDialog());
                    break;
                case PlayerRankKey:
                    player.Enqueue(new S.OpenRankDialog());
                    break;
                case HeroRankKey:
                    player.Enqueue(new S.OpenHeroRankDialog());
                    break;
                case LMSBRKey:
                    player.Enqueue(new S.OpenLMSDialog());
                    break;
            }
        }
...
public override void ApplyPoison(Poison p, MapObject Caster = null, bool NoResist = false, bool ignoreDefence = true)
        ...
        public void CheckVisible(PlayerObject Player, bool Force = false)
        {
            ...
            if (Conq != null && Conq.WarIsOn && !Info.SWOverRide)
            {
