...
namespace Server.MirObjects
{
    ...
    public class GuildObject
    {
        ...
        public int WarWins = 0;
        public int WarLoses = 0;

        public DateTime VoteExpire = DateTime.MinValue;
        public int ProVote = 0;
        public int ConVote = 0;
        public GuildObject PendingGuild = null;
        ...
        public GuildObject(BinaryReader reader)
        {
            ...
            {
                int index = i;
                Ranks.Add(new Rank(reader, true, Envir.LoadVersion) { Index = index });
        ...
        public void SendMessage(string message, ChatType Type = ChatType.Guild, List<UserItem> items = null)
        {
            for (int i = 0; i < Ranks.Count; i++)
                for (int j = 0; j < Ranks[i].Members.Count; j++)
                {
                    ...
                    if (player != null)
                        player.ReceiveChat(message, Type, items);
                }
        }
        ...
        public void AdjustTaxRate(PlayerObject player, byte rate)
        {
            if (player == null)
                return;
            ...
            TaxRate = rate;
            NeedSave = true;
            SendMessage(string.Format("Guild Tax rate adjusted to {0}%", TaxRate), ChatType.Guild);
            Packet p = new ServerPackets.GuildStatus()
            {
                GuildName = Name,
                GuildRankName = player.MyGuildRank != null ? player.Name : "",
                Experience = Experience,
                MaxExperience = MaxExperience,
                MemberCount = Membercount,
                MaxMembers = MemberCap,
                ...
                ItemCount = (byte)StoredItems.Length,
                BuffCount = (byte)0,//(byte)BuffList.Count,
                MyOptions = player.MyGuildRank != null ? player.MyGuildRank.Options : 0,
                MyRankId = player.MyGuildRank != null ? player.MyGuildRank.Index : 256,
                WarLoses = WarLoses,
                WarPoints = WarPoints,
                WarWins = WarWins,
                EnemyGuild = WarringGuilds.Count == 0 ? "None" : WarringGuilds[0].Name,
                TaxRate = TaxRate
            };
            SendServerPacket(p);
        }
        ...
        public bool NewRank(PlayerObject Self)
        {
            ...
            Rank NewRank = new Rank() { Index = NewIndex, Name = String.Format("Rank-{0}", NewIndex), Options = 0 };
            ...
            SendServerPacket(new ServerPackets.GuildMemberChange() { Name = Self.Name, Status = 6, Ranks = NewRankList });
            NeedSave = true;
            return true;
        }
        ...
        {
            if ((RankIndex >= Ranks.Count) || (Option > 8))
            {
            ...
            List<Rank> NewRankList = new List<Rank>();
            NewRankList.Add(Ranks[RankIndex]);
            SendServerPacket(new ServerPackets.GuildMemberChange() { Name = Self.Name, Status = 7, Ranks = NewRankList });
            NeedSave = true;
            return true;
        }
        public bool ChangeRankName(PlayerObject Self, string RankName, byte RankIndex)
        {
            ...
            for (int i = 0; i < Ranks.Count; i++)
                for (int j = 0; j < Ranks[i].Members.Count; j++)
                {
                    ...
                    if (player != null)
                    {
                        player.Enqueue(new ServerPackets.GuildMemberChange() { Name = Self.Info.Name, Status = 7, Ranks = NewRankList });
                        player.GuildMembersChanged = true;
                        if (i == RankIndex)
                            player.BroadcastInfo();
                    }
        ...
        public void MemberDeleted(string name, PlayerObject formermember, bool kickself)
        {
            ...
            if (formermember != null)
            {
                ...
                formermember.Enqueue(new ServerPackets.GuildStatus() { GuildName = "", GuildRankName = "", MyOptions = 0 });
        ...
        public bool HasRoom()
        {
            if (Name == Settings.NewbieName && Membercount < MemberCap)
                return true;
        ...
        {
            if (Name == Settings.NewbieName) return;
    ...
    public class GuildAtWar
    {
        ...
        public int GuildAScore = 0;
        public int GuildBScore = 0;
