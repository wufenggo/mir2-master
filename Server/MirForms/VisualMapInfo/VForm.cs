    ...
    public partial class VForm : Form
    {
        ...
        private void VForm_FormClosing(object sender, FormClosingEventArgs e)
        {
            ...
            for (int i = 0; i < RespawnPanel.Controls.Count; i++)
            {
                try
                {
                    ...
                    {
                        Location = new Point(RespawnControl.X, RespawnControl.Y),
                        MonsterIndex = RespawnControl.MonsterIndex,
                        Spread = RespawnControl.Range,
                        Count = Convert.ToUInt16(RespawnControl.Count.Text),
                        Delay = Convert.ToUInt16(RespawnControl.Delay.Text),
                        RoutePath = RespawnControl.RoutePath
                    };
            ...
            for (int i = 0; i < MiningPanel.Controls.Count; i++)
            {
                try
                {
                    ...
                    {
                        Location = new Point(MineControl.X, MineControl.Y),
                        Mine = MineControl.MineIndex,
                        Size = MineControl.Range
                    };
        ...
        private void InitializeMineInfo()
        {
            ...
            for (int i = 0; i < VisualizerGlobal.MapInfo.MineZones.Count; i++)
            {
                ...
                {
                    Dock = DockStyle.Top,
                    MineIndex = VisualizerGlobal.MapInfo.MineZones[i].Mine,
                    X = VisualizerGlobal.MapInfo.MineZones[i].Location.X,
                    Y = VisualizerGlobal.MapInfo.MineZones[i].Location.Y,
                    tempRange = VisualizerGlobal.MapInfo.MineZones[i].Size,
                    Range = VisualizerGlobal.MapInfo.MineZones[i].Size
                };
        ...
        private void InitializeRespawnInfo()
        {
            ...
            for (int i = 0; i < VisualizerGlobal.MapInfo.Respawns.Count; i++)
            {
                ...
                {
                    Dock = DockStyle.Top,
                    MonsterIndex = VisualizerGlobal.MapInfo.Respawns[i].MonsterIndex,
                    X = VisualizerGlobal.MapInfo.Respawns[i].Location.X,
                    Y = VisualizerGlobal.MapInfo.Respawns[i].Location.Y,
                    Range = VisualizerGlobal.MapInfo.Respawns[i].Spread
                };
