...
namespace Server
{
    public partial class SystemInfoForm : Form
    {
        ...
        public bool FishingChanged = false, MailChanged = false, GoodsChanged = false, RefineChanged = false, MarriageChanged = false, MentorChanged = false, GemChanged = false, SpawnChanged = false, GroupChanged = false;
        ...
        public SystemInfoForm(int selectedTab = 0)
        {
            ...
            if (selectedTab > this.GroupTab.TabCount - 1) selectedTab = 0;

            this.GroupTab.SelectedIndex = selectedTab;
        ...
        {
            AssassinCheckBox.Checked = Settings.GrpSin;
            WarriorCheckBox.Checked = Settings.GrpWar;
            WizardCheckBox.Checked = Settings.GrpWiz;
            TaoistCheckBox.Checked = Settings.GrpTao;
            ArcherCheckBox.Checked = Settings.GrpArc;
            checkBox1.Checked = Settings.GrpNone;
            ExpIncreaseTextBox.Text = Settings.ExpStageIncrease.ToString();
        }
        ...
        private void FishingAttemptsTextBox_TextChanged(object sender, EventArgs e)
        {
            ...
            if (!int.TryParse(ActiveControl.Text, out int temp) || temp > 100 || temp < 10)
            {
        ...
        private void FishingSuccessRateStartTextBox_TextChanged(object sender, EventArgs e)
        {
            ...
            if (!int.TryParse(ActiveControl.Text, out int temp) || temp > 100 || temp < 0)
            {
                ActiveControl.BackColor = Color.Red;
                return;
            }
        ...
        private void FishingSuccessRateMultiplierTextBox_TextChanged(object sender, EventArgs e)
        {
            ...
            if (!int.TryParse(ActiveControl.Text, out int temp) || temp > 100 || temp < 1)
            {
                ActiveControl.BackColor = Color.Red;
                return;
            }
        ...
        private void MailCostPer1kTextBox_TextChanged(object sender, EventArgs e)
        {
            ...
            if (!uint.TryParse(ActiveControl.Text, out uint temp) || temp > 1000)
            {
                ActiveControl.BackColor = Color.Red;
                return;
            }
        ...
        private void MailInsurancePercentageTextBox_TextChanged(object sender, EventArgs e)
        {
            ...
            if (!uint.TryParse(ActiveControl.Text, out uint temp) || temp > 100 || temp < 0)
            {
                ActiveControl.BackColor = Color.Red;
                return;
            }
        ...
        private void LoverBonusEXP_textbox_TextChanged(object sender, EventArgs e)
        {
            ...
            if (!int.TryParse(ActiveControl.Text, out int temp) || temp > 500 || temp < 0)
            {
                ActiveControl.BackColor = Color.Red;
                return;
            }
        ...
        private void MarriageCooldown_textbox_TextChanged(object sender, EventArgs e)
        {
            ...
            if (!int.TryParse(ActiveControl.Text, out int temp) || temp > 365 || temp < 0)
            {
                ActiveControl.BackColor = Color.Red;
                return;
            }
        ...
        private void RequiredLevel_textbox_TextChanged(object sender, EventArgs e)
        {
            ...
            if (!int.TryParse(ActiveControl.Text, out int temp) || temp > 255 || temp < 0)
            {
                ActiveControl.BackColor = Color.Red;
                return;
            }
        ...
        private void BaseChance_textbox_TextChanged(object sender, EventArgs e)
        {
            ...
            if (!byte.TryParse(ActiveControl.Text, out byte temp) || temp > 100 || temp < 0)
            {
                ActiveControl.BackColor = Color.Red;
                return;
            }
        ...
        private void RefineTime_textbox_TextChanged(object sender, EventArgs e)
        {
            ...
            if (!int.TryParse(ActiveControl.Text, out int temp) || temp > 1000 || temp < 0)
            {
                ActiveControl.BackColor = Color.Red;
                return;
            }
        ...
        private void NormalStat_textbox_TextChanged(object sender, EventArgs e)
        {
            ...
            if (!byte.TryParse(ActiveControl.Text, out byte temp) || temp > 255 || temp < 0)
            {
                ActiveControl.BackColor = Color.Red;
                return;
            }
        ...
        private void RefineCost_textbox_TextChanged(object sender, EventArgs e)
        {
            ...
            if (!int.TryParse(ActiveControl.Text, out int temp) || temp > 2000 || temp < 0)
            {
                ActiveControl.BackColor = Color.Red;
                return;
            }
        ...
        private void GoodsMaxStoredTextBox_TextChanged(object sender, EventArgs e)
        {
            ...
            if (!uint.TryParse(ActiveControl.Text, out uint temp) || temp > 500 || temp < 0)
            {
                ActiveControl.BackColor = Color.Red;
                return;
            }
        ...
        private void GoodsBuyBackTimeTextBox_TextChanged(object sender, EventArgs e)
        {
            ...
            if (!uint.TryParse(ActiveControl.Text, out uint temp) || temp > 1440 || temp < 0)
            {
                ActiveControl.BackColor = Color.Red;
                return;
            }
        ...
        private void GoodsBuyBackMaxStoredTextBox_TextChanged(object sender, EventArgs e)
        {
            ...
            //  It's fine for things with limits lower than the highest value for that type
            if (!uint.TryParse(ActiveControl.Text, out uint temp) || temp > 500 || temp < 0)
            {
                ActiveControl.BackColor = Color.Red;
                return;
        ...
        private void WarriorCheckBox_CheckedChanged(object sender, EventArgs e)
        {
            if (ActiveControl != sender) return;
            if (checkBox1.Checked && WarriorCheckBox.Checked)
                checkBox1.Checked = false;
            Settings.GrpWar = WarriorCheckBox.Checked;
            GroupChanged = true;
        }

        private void WizardCheckBox_CheckedChanged(object sender, EventArgs e)
        {
            if (ActiveControl != sender) return;
            if (checkBox1.Checked && WizardCheckBox.Checked)
                checkBox1.Checked = false;
            Settings.GrpWiz = WizardCheckBox.Checked;
        }

        private void TaoistCheckBox_CheckedChanged(object sender, EventArgs e)
        {
            if (ActiveControl != sender) return;
            if (checkBox1.Checked && TaoistCheckBox.Checked)
                checkBox1.Checked = false;
            Settings.GrpTao = TaoistCheckBox.Checked;
            GroupChanged = true;
        }

        private void AssassinCheckBox_CheckedChanged(object sender, EventArgs e)
        {
            if (ActiveControl != sender) return;
            if (checkBox1.Checked && AssassinCheckBox.Checked)
                checkBox1.Checked = false;
            Settings.GrpSin = AssassinCheckBox.Checked;
            GroupChanged = true;
        }

        private void ArcherCheckBox_CheckedChanged(object sender, EventArgs e)
        {
            if (ActiveControl != sender) return;
            if (checkBox1.Checked && ArcherCheckBox.Checked)
                checkBox1.Checked = false;
            Settings.GrpArc = ArcherCheckBox.Checked;
            GroupChanged = true;
        }

        private void ExpIncreaseTextBox_TextChanged(object sender, EventArgs e)
        {
            if (ActiveControl != sender) return;
            ...
            ActiveControl.BackColor = SystemColors.Window;
            Settings.ExpStageIncrease = temp;
            GroupChanged = true;
        ...
        private void checkBox1_CheckedChanged(object sender, EventArgs e)
        {
            ...
            {
                WarriorCheckBox.Checked = false;
                WizardCheckBox.Checked = false;
                TaoistCheckBox.Checked = false;
                AssassinCheckBox.Checked = false;
                ArcherCheckBox.Checked = false;
                Settings.GrpNone = true;
                Settings.GrpArc = false;
                Settings.GrpSin = false;
                Settings.GrpTao = false;
                Settings.GrpWar = false;
                Settings.GrpWiz = false;
            }
            else
            {
                Settings.GrpNone = false;
                Settings.GrpArc = false;
                Settings.GrpSin = false;
                Settings.GrpTao = false;
                Settings.GrpWar = false;
        ...
        private void txtSpawnTickUsers_TextChanged(object sender, EventArgs e)
        {
            ...
            if (!int.TryParse(ActiveControl.Text, out int temp) || (temp < 0))
            {
                ActiveControl.BackColor = Color.Red;
                return;
            }
        ...
        private void txtSpawnTickSpeed_TextChanged(object sender, EventArgs e)
        {
            ...
            if (!double.TryParse(ActiveControl.Text, out double temp) || (temp <= 0) || (temp > 1.0))
            {
                ActiveControl.BackColor = Color.Red;
                return;
            }
