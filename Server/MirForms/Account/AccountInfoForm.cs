...
namespace Server
{
    public partial class AccountInfoForm : Form
    {
        ...
        public void RefreshInterface(int type = 0)
        {
            if (InvokeRequired)
            {
                Invoke((MethodInvoker)delegate
                {
                    RefreshInterface(type);
                });
                    return;
            }
            ...
            {
                accounts = SMain.Envir.MatchAccountsByCreationIP(CreationIPTextBox.Text, MatchFilterCheckBox.Checked);
            }

            else if (type == 2)
            {
                accounts = SMain.Envir.MatchAccountsByLastIP(LastIPTextBox.Text, MatchFilterCheckBox.Checked);
            }
            ...
            ChatBannedCheckBox.CheckState = info.ChatBanned ? CheckState.Checked : CheckState.Unchecked;
            ...
            DevAccount.CheckState = info.DevAccount ? CheckState.Checked : CheckState.Unchecked;
            ...
            for (int i = 0; i < _selectedAccountInfos.Count; i++)
            {
                ...
                if (ChatBannedCheckBox.Checked != info.ChatBanned) ChatBannedCheckBox.CheckState = CheckState.Indeterminate;
                ...
                if (DevAccount.Checked != info.DevAccount)
                    DevAccount.CheckState = CheckState.Indeterminate;
            }
        }
        ...
        private void ChatBannedCheckBox_CheckedChanged(object sender, EventArgs e)
        {
            if (ActiveControl != sender) return;
            ...
            for (int i = 0; i < _selectedAccountInfos.Count; i++)
            {
                _selectedAccountInfos[i].ChatBanned = ChatBannedCheckBox.Checked;
                _selectedAccountInfos[i].Update();
            }
            AutoResize();
            AccountInfoListView.EndUpdate();
        ...
        private void LastIPButton_Click(object sender, EventArgs e)
        {
            RefreshInterface(2);
        }

        private void CreateIPButton_Click(object sender, EventArgs e)
        {
            RefreshInterface(1);
        }
        ...
        private void DevAccount_CheckChanged(object sender, EventArgs e)
        {
            ...
            for (int i = 0; i < _selectedAccountInfos.Count; i++)
            {
                _selectedAccountInfos[i].DevAccount = DevAccount.CheckState == CheckState.Checked ? true : false;
        ...
        {
            if (ActiveControl != sender) return;
            if (DevAccount.Checked)
                DevAccount.Checked = false;
            AccountInfoListView.BeginUpdate();
            for (int i = 0; i < _selectedAccountInfos.Count; i++)
            {
