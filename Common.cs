...
public enum RequiredClass : byte
{
    ...
    Assassin = 8,
    Archer = 16,
    战法道  = Warrior | Wizard | Taoist,
    通用  = 战法道 | Assassin | Archer
}
[Flags]
[Obfuscation(Feature = "renaming", Exclude = true)]
...
public static class Functions
{
    ...
    public static string PrintTimeSpanFromSeconds(double secs, bool accurate = true)
    {
        ...
        {
            answer = accurate ? string.Format("{0}天 {1:D2}时 {2:D2}分 {3:D2}秒", (int)t.Days, (int)t.Hours, t.Minutes, t.Seconds) : string.Format("{0}d {1}时 {2:D2}分", (int)t.TotalDays, (int)t.Hours, t.Minutes);
        }
...
public class GuildBuffInfo
{
    ...
    public string ShowStats()
    {
        ...
        if (BuffHpRegen > 0)
        {
            text += string.Format("增加生命恢复 by: {0}.", BuffHpRegen);
            if (text != "") text += "\n";
        }
        if (BuffMPRegen > 0)
        {
            text += string.Format("增加魔法恢复 by: {0}.", BuffMPRegen);
            if (text != "") text += "\n";
        }
        if (BuffMineRate > 0)
        {
            text += string.Format("增加 采矿率 by: {0}%.", BuffMineRate * 5);
            if (text != "") text += "\n";
        }
        if (BuffGemRate > 0)
        {
            text += string.Format("增加 宝石成功 by: {0}%.", BuffGemRate * 5);
            if (text != "") text += "\n";
        }
        if (BuffFishRate > 0)
        {
            text += string.Format("增加 钓鱼成功率 by: {0}%.", BuffFishRate * 5);
            if (text != "") text += "\n";
        }
        if (BuffExpRate > 0)
        {
            text += string.Format("增加 经验 by: {0}%.", BuffExpRate);
            if (text != "") text += "\n";
        }
        if (BuffCraftRate > 0)
        {
            text += string.Format("增加 打造成功 by: {0}%.", BuffCraftRate * 5);
            if (text != "") text += "\n";
        }
        if (BuffSkillRate > 0)
        {
            text += string.Format("增加技能值 by: {0}.", BuffSkillRate);
            if (text != "") text += "\n";
        }
        if (BuffAttack > 0)
        {
            text += string.Format("增加伤害 by: {0}.", BuffAttack);
            if (text != "") text += "\n";
        }
        if (BuffDropRate > 0)
        {
            text += string.Format("爆率增加 by: {0}%.", BuffDropRate);
            if (text != "") text += "\n";
        }
        if (BuffGoldRate > 0)
        {
            text += string.Format("金币掉率 by: 0-{0}.", BuffGoldRate);
            if (text != "") text += "\n";
        }
