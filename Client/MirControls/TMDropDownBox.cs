...
{
    public class TMDropDownBox : MirControl
    {
        private MirLabel _label;
        private MirButton _DropDownButton;
        private MirLabel[] _Option = new MirLabel[25];
        ...
        public TMDropDownBox()
        {
            ...
            for (int i = 0; i < _Option.Length; i++)
            {
                ...
                int index = i;
                _Option[index].MouseEnter += (o, e) => _Option[index].BackColour = Color.LightGray;
        ...
        {
            Size = new Size(Size.Width, OrigHeight + (25 * 15));
            ScrollIndex = 0;
            ...
            _ScrollDown.Location = new Point(Size.Width - 12, 55);
        }
        ...
        {
            if (ScrollIndex < (Items.Count - 25)) ScrollIndex++;
            Update();
        }
        void ScrollPosition(object sender, MouseEventArgs e)
        {
            ...
            h = (int)((y - 20) / (h / (float)(Items.Count - 1)));
            if (h > Items.Count - 25) h = Math.Max(0, Items.Count - 25);
            if (h != ScrollIndex)
            {
                ScrollIndex = h;
                Update();
