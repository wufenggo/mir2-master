...
{
    public sealed class MirAmountBoxDual : MirImageControl
    {
        public MirLabel TitleLabel, TextLabel;
        public MirButton OKButton, CancelButton, CloseButton;
        public MirTextBox InputTextBox;
        public MirTextBox InputTextBox2;
        public MirControl ItemImage;
        public int ImageIndex;
        public uint Amount,Amount2, MinAmount, MaxAmount;

        public MirAmountBoxDual(string title, int image, uint max, uint min = 0, uint defaultAmount = 0)
        {
            ImageIndex = image;
            MaxAmount = max;
            MinAmount = min;
            ...
            InputTextBox2.TextBox.KeyPress += MirInputBox_KeyPress;
            InputTextBox2.Text = (defaultAmount > 0 && defaultAmount <= MaxAmount) ? defaultAmount.ToString() : MaxAmount.ToString();
            InputTextBox2.TextBox.SelectionStart = 0;
            InputTextBox2.TextBox.SelectionLength = InputTextBox2.Text.Length;

            InputTextBox2.TextBox.TextChanged += TextBox_TextChanged2;
            InputTextBox.TextBox.TextChanged += TextBox_TextChanged;

        }
        public MirAmountBoxDual(string title, int image, string message)
        {
            ImageIndex = image;
        ...
        void TextBox_TextChanged(object sender, EventArgs e)
        {
            if (uint.TryParse(InputTextBox.Text, out Amount) && Amount >= MinAmount)
            {
                ...
                if ((uint.TryParse(InputTextBox2.Text, out Amount2) && Amount2 >= 0) || (uint.TryParse(InputTextBox.Text, out Amount) && Amount >= MinAmount))
            ...
            else
            {
                ...
                if (!uint.TryParse(InputTextBox2.Text, out Amount2) && Amount2 >= 0)
        ...
        void TextBox_TextChanged2(object sender, EventArgs e)
        {
            if (uint.TryParse(InputTextBox2.Text, out Amount2) && Amount2 >= 0)
            {
                InputTextBox2.BorderColour = Color.Lime;

                if ((uint.TryParse(InputTextBox.Text, out Amount) && Amount >= MinAmount) || (uint.TryParse(InputTextBox2.Text, out Amount2) && Amount2 >= 0))
                    OKButton.Visible = true;
                ...
                {
                    Amount2 = MaxAmount;
                    InputTextBox2.Text = MaxAmount.ToString();
                    InputTextBox2.TextBox.SelectionStart = InputTextBox2.Text.Length;
                }

                if (Amount2 == MaxAmount)
                    InputTextBox2.BorderColour = Color.Orange;
            }
            else
            {
                InputTextBox2.BorderColour = Color.Red;
