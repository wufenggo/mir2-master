...
namespace Client.MirScenes.Dialogs
{
    ...
    {
        private readonly MirButton _shareButton, _pauseButton, _cancelButton, _finishButton;
        ...
        public QuestDetailDialog()
        {
            ...
            _finishButton.Click += (o, e) =>
            {
                if (Reward == null || !Quest.Completed) return;

                if (Reward.SelectedItemIndex < 0 && Quest.QuestInfo.RewardsSelectItem.Count > 0)
                {
                    MirMessageBox messageBox = new MirMessageBox("You must select a reward item.");
                    messageBox.Show();
                    return;
                ...
                Network.Enqueue(new C.FinishQuest { QuestIndex = Quest.QuestInfo.Index, SelectedItemIndex = Reward.SelectedItemIndex });
                Hide();
            };
        ...
        public void DisplayQuestDetails(ClientQuestProgress quest)
        {
            ...
            if (quest.QuestInfo.autoComplete && quest.Completed)
            {
                _finishButton.Visible = true;
    ...
    public sealed class QuestDiaryDialog : MirImageControl
    {
        ...
        public List<AvailableGroupQuestItem> AvailableGroups = new List<AvailableGroupQuestItem>();
        //  Completed
        public List<ClientQuestCompleted> CompletedQuests = new List<ClientQuestCompleted>();        
        public List<CompletedQuestItems> CompletedTasks = new List<CompletedQuestItems>();


        public List<string> ExpandedGroups = new List<string>();

        private MirButton _currrentButton, _availableButton, _completedButton;
        ...
        public QuestDiaryDialog()
        {
            ...
            _currrentButton.Click += (o, e) => UpdateInterface(0);

            _availableButton = new MirButton
            {
                Index = 821,
                HoverIndex = 822,
                PressedIndex = 823,
                ...
                Location = new Point(120, 436),
                Sound = SoundList.ButtonA,
                Hint = "Available Quests"
            };
            _availableButton.Click += (o, e) => UpdateInterface(1);
            _completedButton = new MirButton
            {
                Index = 818,
                HoverIndex = 819,
                PressedIndex = 820,
                ...
                Location = new Point(200, 436),
                Sound = SoundList.ButtonA,
                Hint = "Compeleted Quests"
            };
            _completedButton.Click += (o, e) => UpdateInterface(2);
            MirButton closeButton = new MirButton
            {
                Index = 360,
                HoverIndex = 361,
        ...
        public void UpdateInterface(byte page = 0)
        {
            ClearLog();
            ...
            switch(page)
            {
                ...
                case 1:
                    {
                        AvailableQuests = GameScene.User.AvailableQuests;
                        _takenQuestsLabel.Text = string.Format("List: {0}/{1}", AvailableQuests.Count, ushort.MaxValue);

                        var groupedQuests = AvailableQuests.GroupBy(d => d.Group).ToList();

                        int nextY = 40;

                        foreach (var group in groupedQuests)
                        {
                            List<ClientAvailableQuests> singleGroup = @group.ToList();

                            bool expanded = false;

                            AvailableGroupQuestItem groupQuest = new AvailableGroupQuestItem(group.Key, singleGroup, expanded)
                            {
                                Parent = this,
                                Visible = true,
                            ...
                            groupQuest.ExpandedChanged += (o, e) =>
                            {
                                ...
                                foreach (AvailableGroupQuestItem task in AvailableGroups)
                                {
                            ...
                            {
                                AvailableSingleQuestItem singleQuestItem = (AvailableSingleQuestItem)o;

                                if (singleQuestItem == null) return;

                                foreach (AvailableGroupQuestItem item in AvailableGroups)
                                {
                                    item.DeselectQuests();
                                }
                            ...
                            AvailableGroups.Add(groupQuest);
                        }
                    }
                    break;
                case 2:
                    {
                        CompletedQuests = GameScene.User.ClientCompletedQuests;
                        _takenQuestsLabel.Text = string.Format("List: {0}/{1}", CompletedQuests.Count, byte.MaxValue);
                        var groupedQuests = CompletedQuests.GroupBy(d => d.Category).ToList();
                        int nextY = 40;
                        foreach (var group in groupedQuests)
                        {
                            List<ClientQuestCompleted> singleGroup = group.ToList();
                            bool expanded = false;
                            CompletedQuestItems groupQuest = new CompletedQuestItems(group.Key, singleGroup, expanded)
                            {
                                Parent = this,
                                Visible = true,
                                Location = new Point(15, nextY)
                            ...
                            groupQuest.ExpandedChanged += (o, e) =>
                            {
                                ...
                                foreach (CompletedQuestItems task in CompletedTasks)
                                {
                                    task.Location = new Point(15, nextY);
                                    nextY += task.SizeY;
                            ...
                            groupQuest.SelectedQuestChanged += (o, e) =>
                            {
                                CompletedQuestItems singleQuestItem = (CompletedQuestItems)o;

                                if (singleQuestItem == null) return;

                                foreach (CompletedQuestItems item in CompletedTasks)
                                {
                                    item.DeselectQuests();
                                }
        ...
        public void ClearLog()
        {
            ...
            foreach (CompletedQuestItems items in CompletedTasks)
            {
                items.Dispose();
            }
            foreach (AvailableGroupQuestItem avail in AvailableGroups)
            {
                avail.Dispose();
            }
    ...
    public sealed class QuestTrackingDialog : MirImageControl
    {
        ...
        public List<MirAnimatedButton> TaskButtons = new List<MirAnimatedButton>();

        public Font QuestFont = new Font(Settings.FontName, 8F);

        public MirAnimatedButton FinishQuest;
        ...
        public void DisplayQuests()
        {
            ...
            for (int i = 0; i < questsToTrack.Count; i++)
            {
                ...
                string questName = questsToTrack[i].QuestInfo.Name;

                if (questsToTrack[i].QuestInfo.time > 0)
                {
                    var sec = questsToTrack[i].EndTime % 60;
                    var min = questsToTrack[i].EndTime / 60;

                    questName += "(" + string.Format("{0} Minutes {1} Seconds", min, sec) + ")";
                }
                ...
                _questNameLabel = new MirLabel
                {
                    ...
                    Font = QuestFont,
                    ForeColour = questsToTrack[i].QuestInfo.Hero ? Color.MediumPurple : Color.Yellow,
                ...
                var qb = TaskButtons.FirstOrDefault(x => x.HoldValue == questsToTrack[i].Id);

                if (qb == null)
                {
                    FinishQuest = new MirAnimatedButton
                    {
                        Animated = true,
                        AnimationCount = 2,
                        Loop = false,
                        ...
                        Location = new Point(5 + TextRenderer.MeasureText(CMain.Graphics, questName, QuestFont).Width, 120 + y),
                        Sound = SoundList.ButtonA,
                        Visible = questsToTrack[i].Completed && questsToTrack[i].QuestInfo.autoComplete,
                        HoldValue = questsToTrack[i].Id,
                        Size = new Size(100, 100),

                    };
                    FinishQuest.Click += (o, e) => OpenQuestWindow(((MirAnimatedButton)o).HoldValue);

                    TaskButtons.Add(FinishQuest);
                }
                else
                {
                    FinishQuest.Location = new Point(5 + TextRenderer.MeasureText(CMain.Graphics, questName, QuestFont).Width, 120 + y);
                    FinishQuest.Visible = questsToTrack[i].Completed && questsToTrack[i].QuestInfo.autoComplete;
                }
                ...
                foreach (string questToTrack in questsToTrack[i].TaskList)
                {
                    ...
                    _questTaskLabel = new MirLabel
                    {
                        ...
                        BackColour = Color.Transparent,
                        Font = QuestFont,
                        ForeColour = trackedQuest.Contains("(Completed)") ? Color.YellowGreen : Color.White,
            ...
            for (int i = TaskButtons.Count - 1; i > 0; i--)
            {
                var q = questsToTrack.FirstOrDefault(x => x.Id == TaskButtons[i].HoldValue);
                if (q == null)
                {
                    TaskButtons[i].Dispose();
                    TaskButtons.Remove(TaskButtons[i]);
                }
            }
        ...
        public void OpenQuestWindow(int id)
        {
            ...
            foreach (var tg in GameScene.Scene.QuestLogDialog.TaskGroups)
            {
                foreach (var q in tg._tasks)
                {
                    if (q.Quest.Id == id)
                    {
                        GameScene.Scene.QuestDetailDialog.DisplayQuestDetails(q.Quest);
                        break;
                    }
                }
            }
        ...
        public void UpdateTrackedQuests()
        {
            ...
            // CMain.InputKeys.Save(); - root of lag JP 01/07/016
        }
    ...
    public sealed class QuestMessage : MirControl
    {
        ...
        private void QuestMessage_AfterDraw(object sender, EventArgs e)
        {
            ...
            for (int i = TopLine; i < BottomLine; i++)
            {
                ...
                Libraries.CustomPrguse.Draw(919, new Point(DisplayLocation.X + 5, DisplayLocation.Y + 5 + (i - TopLine) * 15 + adjust), Color.White);
    ...
    {
        private readonly MirLabel _goldLabel, _expLabel, _creditLabel, _itemexpLabal;
        ...
        public static MirImageControl[] BuffRewards = new MirImageControl[4];
        ...
        void QuestReward_BeforeDraw(object sender, EventArgs e)
        {
            ...
            if (quest.RewardCredit > 0)
                Libraries.Prguse.Draw(541, DisplayLocation.X + 160 + creditXOffset, DisplayLocation.Y + 2);
            else
            {
                itemexpXOffset -= 90;
            }
            if (quest.ItemEXPReward > 0)
                Libraries.Prguse.Draw(966, DisplayLocation.X + 230 + itemexpXOffset, DisplayLocation.Y + 2);
        ...
        public void CleanRewards()
        {
            ...
            foreach (MirImageControl item in BuffRewards.Where(item => item != null))
            {
                item.Dispose();
            }
        ...
        public void UpdateInterface()
        {
            ...
            if (quest.RewardExp > 0)
            {
                _expLabel.Text = quest.percentageExp ? quest.RewardExp.ToString() + " %" : quest.RewardExp.ToString();
            ...
            if (quest.RewardCredit > 0)
            {
                ...
                _creditLabel.Location = new Point(186 + creditXOffset, 0);
            ...
            if (quest.ItemEXPReward > 0)
            {
                _itemexpLabal.Text = quest.percentageExp ? quest.ItemEXPReward.ToString() + " %" : quest.ItemEXPReward.ToString();
                _itemexpLabal.Location = new Point(262 + itemexpXOffset, 0);
                _itemexpLabal.Visible = true;
            }
            ...
            if (quest.RewardsFixedItem.Count > 0)
            {
                ...
                for (int i = 0; i < FixedItems.Length; i++)
                {
                    ...
                    FixedItems[i] = new QuestCell
                    {
                        ...
                        Parent = this,
                        Location = new Point(i * 45 + 15, 25),
                        Fixed = true
                    };
                }
            }

            if (quest.BuffRewards.Count > 0)
            {

                for (int i = 0; i < BuffRewards.Length; i++)
                {
                    if (i >= quest.BuffRewards.Count) break;

                    BuffRewards[i] = new MirImageControl
                    {
                        Index = GameScene.Scene.BuffImage(quest.BuffRewards[i].buff),
                        Hint = "Buff: " + quest.BuffRewards[i].buff.ToString() + "\nDuration: " + quest.BuffRewards[i].time.ToString() + "m",
                        Library = Libraries.BuffIcon,
                        Parent = this,
                        Location = new Point(i * 24 + 14, 26), //24
                        Visible = true,
                    };
                }
            }
            ...
            {
                var selRewards = FilterRewards(quest.RewardsSelectItem, quest.Hero);
        ...
        public List<QuestItemReward> FilterRewards(List<QuestItemReward> rewardItems, bool hero)
        {
        ...
        protected override void Dispose(bool disposing)
        {
            ...
            foreach (MirImageControl cell in BuffRewards.Where(cell => cell != null && !cell.IsDisposed))
            {
                cell.Dispose();
            }
    ...
    public sealed class QuestCell : MirControl
    {
        ...
        protected internal override void DrawControl()
        {
            ...
            if (Fixed)
            {
                Libraries.CustomPrguse.Draw(989, DisplayLocation.X, DisplayLocation.Y - 1);
            }
            else if (Selected)
            {
                Libraries.CustomPrguse.Draw(979, DisplayLocation.X, DisplayLocation.Y - 5);
            }
    ...
    public sealed class CompletedQuestItems : MirControl
    {
        public string Group = string.Empty;
        public List<ClientQuestCompleted> Quests = new List<ClientQuestCompleted>();
        public bool Expanded = true;
        public int SizeY = 15;
        private readonly MirButton _expandButton;
        private readonly MirLabel _groupLabel;
        public readonly List<CompletedQuestItem> _tasks = new List<CompletedQuestItem>();
        public bool Selected;
        ...
        public CompletedQuestItems(string group, List<ClientQuestCompleted> quests, bool expanded)
        {
            Group = group;
            Quests = quests;
            Expanded = expanded;
            ...
            {
                CompletedQuestItem singleQuest = new CompletedQuestItem(Quests[i])
                {
                    Parent = this,
        ...
        {
            foreach (CompletedQuestItem task in _tasks)
            {
                task.Dispose();
            }
            _tasks.Clear();
    ...
    public sealed class AvailableGroupQuestItem : MirControl
    {
        public string Group = string.Empty;
        public List<ClientAvailableQuests> Quests = new List<ClientAvailableQuests>();
        ...
        private readonly MirButton _expandButton;
        private readonly MirLabel _groupLabel;
        public readonly List<AvailableSingleQuestItem> _tasks = new List<AvailableSingleQuestItem>();
        ...
        public AvailableGroupQuestItem(string group, List<ClientAvailableQuests> quests, bool expanded)
        {
            Group = group;
            Quests = quests;
            Expanded = expanded;
            ...
            for (int i = 0; i < Quests.Count; i++)
            {
                bool Track = Settings.TrackedQuests.Contains(Quests[i].Index) ? true : false;
                AvailableSingleQuestItem singleQuest = new AvailableSingleQuestItem(Quests[i])
                {
                    Parent = this,
                    Location = new Point(18, (15 * (i + 1))),
                    Size = new Size(280, 15),
        ...
        public void ClearTasks()
        {
            foreach (AvailableSingleQuestItem task in _tasks)
            {
                task.Dispose();
            }
            _tasks.Clear();
    ...
    public sealed class QuestGroupQuestItem : MirControl
    {
        ...
        private readonly MirButton _expandButton;
        private readonly MirLabel _groupLabel;
        public readonly List<QuestSingleQuestItem> _tasks = new List<QuestSingleQuestItem>();
    ...
    public sealed class CompletedQuestItem : MirControl
    {
        ...
        public CompletedQuestItem(ClientQuestCompleted quest)
        {
            Quest = quest;
            ...
            {
                Text = string.Format("{0} {1:dd-MM-yy}", name, quest.CompleteDateTime),
                AutoSize = true,
                Font = new Font(Settings.FontName, 8F),
            ...
            _stateLabel = new MirLabel
            {
                ...
                ForeColour = Color.Lime,
                Parent = this,
                Location = new Point(_questLabel.Location.X + _questLabel.Size.Width + 2, 0),
                Sound = SoundList.ButtonA
            };
        }
    ...
    public sealed class AvailableSingleQuestItem : MirControl
    {
        private MirLabel _questLabel, _stateLabel;
        private readonly MirImageControl _selectedImage;

        public ClientAvailableQuests Quest;
        ...
        public AvailableSingleQuestItem(ClientAvailableQuests quest)
        {
            Quest = quest;
            Size = new Size(250, 15);

            string name = Quest.Name;
            string level = string.Format("Lv{0}", Quest.MinLevel);
            string state = "Available";

            bool lowLevelQuest = (MapObject.User.Level - quest.MinLevel) > 10;
            ...
            _questLabel = new MirLabel
            {
                ...
                AutoSize = true,
                Font = new Font(Settings.FontName, 8F),
                ForeColour = lowLevelQuest ? Color.Gray : Color.White,
                Parent = this,
                Location = new Point(0, 0),
                Sound = SoundList.ButtonA
            };
