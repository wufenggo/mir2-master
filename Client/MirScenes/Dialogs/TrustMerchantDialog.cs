...
namespace Client.MirScenes.Dialogs
{
    public sealed class TrustMerchantDialog : MirImageControl
    {
        ...
        public TrustMerchantDialog()
        {
            ...
            BuyButton.Click += (o, e) =>
            {
                ...
                if (UserMode)
                {
                    ...
                    {
                        MirMessageBox box = new MirMessageBox(string.Format("{0} 还没售出，你确定要拿回吗？", Selected.Listing.Item.FriendlyName), MirMessageBoxButtons.YesNo);
                ...
                {
                    MirMessageBox box = new MirMessageBox(string.Format("你确定要用{1}购买{0}吗？", Selected.Listing.Item.FriendlyName, Selected.Listing.Price), MirMessageBoxButtons.YesNo);
                    box.YesButton.Click += (o1, e2) =>
                    {
                        MarketTime = CMain.Time + 3000;
        ...
        {
            SellerLabel.Text = UserMode ? "物品状态" : "卖家";
            ...
            DateLabel.Text = string.Format("上架日期: {0}", Selected.Listing.ConsignmentDate);
            ExpireLabel.Text = string.Format("下架日期: {0}", Selected.Listing.ConsignmentDate.AddDays(Globals.ConsignmentLength));
        ...
        private void SearchTextBox_KeyPress(object sender, KeyPressEventArgs e)
        {
            if (CMain.Time < SearchTime)
            {
                GameScene.Scene.ChatDialog.ReceiveChat(string.Format("再次搜索 {0} 秒.", Math.Ceiling((SearchTime - CMain.Time) / 1000D)), ChatType.System);
                return;
            }
