...
namespace Client.MirScenes.Dialogs
{
    public class MailListDialog : MirImageControl
    {
        ...
        MirButton SendButton, ReplyButton, ReadButton, DeleteButton, BlockListButton, BugReportButton, DeleteAllButton;
                ...
                for(int i = Rows.Length - 1; i >= 0; i--)
                {
                        ...
                        Rows[i].Mail != null)
                    {
                        if (Rows[i].Mail.Items.Count > 0 || Rows[i].Mail.Gold > 0)
                        {
                            ...
                            messageBox.YesButton.Click += (o1, e1) =>                            
                                Network.Enqueue(new C.DeleteMail { MailID = Rows[i].Mail.MailID });                            
    ...
    public class MailItemRow : MirControl
    {
        ...
        public void UpdateInterface()
        {
            ...
            {
                IconImage.Index = 541;
                IconImage.Library = Libraries.CustomPrguse;
            }
            else
            {
                IconImage.Index = 540;
    ...
    public class MailComposeLetterDialog : MirImageControl
    {
        ...
        public MailComposeLetterDialog()
        {
            ...
            SendButton.Click += (o, e) =>
            {
                var name = RecipientNameLabel.Text.Split(',');

                foreach (var n in name)
                {
                    Network.Enqueue(new C.SendMail { Name = n, Message = MessageTextBox.Text });
