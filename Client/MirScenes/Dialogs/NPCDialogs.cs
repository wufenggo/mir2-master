...
namespace Client.MirScenes.Dialogs
{
    public sealed class NPCDialog : MirImageControl
    {
        ...
        public NPCDialog()
        {
            ...
            helpButton.Click += (o, e) => GameScene.Scene.HelpDialog.DisplayPage("采购");
        }
        ...
        private void BuyItem()
        {
            ...
            else
            {
                ...
                for (int i = 0; i < MapObject.User.Inventory.Length; i++)
                {
                    ...
                    {
                        GameScene.Scene.ChatDialog.ReceiveChat("您不能购买更多的物品.", ChatType.System);
                        return;
                    }
                }
        ...
        private void Confirm()
        {
            ...
            switch (PType)
            {
                case PanelType.Sell:
                    ...
                    {
                        GameScene.Scene.ChatDialog.ReceiveChat("不能卖掉这个物品.", ChatType.System);
                        return;
                    }
                    ...
                    GameScene.Scene.ChatDialog.ReceiveChat("无法在携带更多金币了.", ChatType.System);
                    break;
                ...
                case PanelType.Consign:
                    ...
                    MirAmountBox box = new MirAmountBox("货物的价格:", TargetItem.Image, Globals.MaxConsignment, Globals.MinConsignment)
                    {
                ...
                case PanelType.Refine:
                    ...
                    for (int i = 0; i < GameScene.Scene.RefineDialog.Grid.Length; i++)
                    {
                        if (GameScene.Scene.RefineDialog.Grid[i].Item != null)
                        {
                            ...
                            GameScene.Scene.ChatDialog.ReceiveChat(String.Format("你没有足够的金币来提炼你的 {0}.", TargetItem.FriendlyName), ChatType.System);
                            return;
                        }

                    }
                    GameScene.Scene.ChatDialog.ReceiveChat(String.Format("你没有存放任何物品来完善你的{0}.", TargetItem.FriendlyName), ChatType.System);
                    break;
                case PanelType.CheckRefine:

                    if (TargetItem.RefineAdded == 0)
                    {
                        GameScene.Scene.ChatDialog.ReceiveChat(String.Format("你的{0}还没有被改进，所以不需要检查.", TargetItem.FriendlyName), ChatType.System);
                        return;
                    }
        ...
        public void ItemCell_Click()
        {
            ...
            else
            {
                if (Items[0].Awake.getAwakeLevel() == 0)
                {
                    ...
                    {
                        SelectAwakeType.Items.Add("英勇符文");
                        SelectAwakeType.Items.Add("魔法符文");
                        SelectAwakeType.Items.Add("灵魂符文");
                    }
                    else if (Items[0].Info.Type == ItemType.Helmet)
                    {
                        SelectAwakeType.Items.Add("防御符文");
                        SelectAwakeType.Items.Add("邪恶符文");
                    }
                    else
                    {
                        SelectAwakeType.Items.Add("体力符文");
                    }
                }
                else
                {
    ...
    public sealed class CraftDialog : MirImageControl
    {
        ...
        private void CraftItem()
        {
            ...
            if (max > 1)
            {
                MirAmountBox amountBox = new MirAmountBox("工艺数量:", RecipeItem.Info.Image, max, 0, max);
                ...
                amountBox.OKButton.Click += (o, e) =>
                {
                    if (amountBox.Amount > 0)
                    {
                        ...
                        {
                            GameScene.Scene.ChatDialog.ReceiveChat("你没有所需的材料.", ChatType.System);
                            return;
                        }
