...
namespace Client.MirObjects
{
    class NPCObject : MapObject
    {
        ...
        public int BaseIndex, FrameIndex, FrameInterval, 
            ...
            BaseFrameIndex, BaseFrameInterval;
        ...
        public void Load(S.ObjectNPC info)
        {
            ...
            switch (info.Image)
            {
                ...
                case 244:
                case 245:
                    Frames = FrameSet.NPCs[25];
                    CanChangeDir = true;
                    break;
                case 246:
                    Frames = FrameSet.NPCs[26];
                    CanChangeDir = false;
                    break;
                case 247:
                case 252:
                    Frames = FrameSet.NPCs[27];
                    CanChangeDir = false;
                    break;
                case 248:
                    Frames = FrameSet.NPCs[28];
                    CanChangeDir = false;
                    break;
                case 249:
                case 250:
                case 251:
                    Frames = FrameSet.NPCs[29];
                    CanChangeDir = false;
                    break;
                    #endregion
            }
    ...
    public override void Process()
        {
            ...
            else
            {
                ...
                DrawBaseFrame = Frame.BaseStart + (Frame.BaseOffSet * (byte)Direction) + BaseFrameIndex;
            }
        ...
        public virtual void ProcessFrames()
        {
            ...
            switch (CurrentAction)
            {
                ...
                case MirAction.Harvest:
                    ...
                    if (BaseFrameInterval > 0)
                    if (CMain.Time >= NextMotion3)
                    {
                        GameScene.Scene.MapControl.TextureValid = false;

                        if (SkipFrames) UpdateFrame3();

                        if (UpdateFrame3() >= Frame.BaseCount)
                            BaseFrameIndex = Frame.BaseCount - 1;
                        else
                            NextMotion3 += BaseFrameInterval;
                    }
                    break;
        ...
        public int UpdateFrame3()
        {
            if (Frame == null) return 0;
            if (Frame.Reverse) return Math.Abs(--BaseFrameIndex);
        ...
        public virtual void SetAction()
        {
            if (ActionFeed.Count == 0)
            {
                ...
                BaseFrameInterval = Frame.BaseInterval;
            }
            else
            {
            ...
            NextMotion3 = CMain.Time + BaseFrameInterval;
            GameScene.Scene.MapControl.TextureValid = false;
        ...
        public override void DrawEffects(bool effectsEnabled)
        {
            ...
            if (DrawBaseFrame > 0)
                BodyLibrary.DrawBlend(DrawBaseFrame, DrawLocation, Color.White, true);
